...
public class FriendsDenyCommand extends AbstractAdvancedCommand {

    private UserHandler userHandler;
    private FriendRequestHandler requestHandler;
    private I18n i18n;

    FriendsDenyCommand(UserHandler userHandler, FriendRequestHandler requestHandler, I18n i18n) {
        super(new String[]{"deny"},
                "/<command> <player>",
                "",
                "",
                "No Permission.",
    ...
    public boolean execute(CommandContext context) {
        ...
        addCallback(ListenableFutureUtils.combine(this.userHandler.findOne(sender.getUniqueId().toString()), this.userHandler.findOne(target.getUniqueId().toString())), (data) -> {
            ...
            this.requestHandler.denyFriendRequest(sender, from, to);
        });
        return true;
    }
}
